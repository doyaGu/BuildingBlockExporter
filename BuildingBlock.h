#ifndef NEMO2_BUILDINGBLOCK_H
#define NEMO2_BUILDINGBLOCK_H
...
struct BuildingBlockParameter
{
    ...
    BuildingBlockParameter(const char *name, CKGUID type, const char *value) : name(name), type(type), value(value) {}

    BuildingBlockParameter(std::string name, CKGUID type, std::string value) : name(std::move(name)), type(type), value(std::move(value)) {}
...
public:
    ...
    bool Dump(const char *filename = "", const char *dir = "");
